	<?php
	$sql_moinhat="select * from sanpham order by idsanpham desc limit 0,12";
	$row_moinhat=mysql_query($sql_moinhat);
	
	?>
	<div class="tieude">NEW PRODUCTS</div>
	<ul class="product">
		<?php
		while($dong_moinhat=mysql_fetch_array($row_moinhat)){
			...
			<li><a href="?quanly=chitietsp&idloaisp=<?php echo $dong_moinhat['loaisp'] ?>&id=<?php echo $dong_moinhat['idsanpham'] ?>">
				...
				height:30px; line-height:30px;margin-left:35px;margin-bottom:5px;"><?php echo number_format($dong_moinhat['giadexuat']).' '.'USD'?></p>
			</a></li>
			<?php
		}
		?>
			...
			<ul class="product">
				...
				while($dong=mysql_fetch_array($row)){
					...
					<li><a href="?quanly=chitietsp&idloaisp=<?php echo $dong['loaisp'] ?>&id=<?php echo $dong['idsanpham'] ?>">
						...
						<p style="color:red;font-weight:bold; border:1px solid #d9d9d9; width:150px;
						height:30px; line-height:30px;margin-left:35px;margin-bottom:5px;"><?php echo number_format($dong['giadexuat']).' '.'USD' ?></p>
					</a></li>
					<?php
				}
			}else{
				echo '<h3 style="margin:5px;font-style:italic;color:#000">NO PRODUCTS</h3>';
			}
			?>
		</ul>
		<div class="clear"></div>
